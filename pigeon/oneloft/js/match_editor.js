    ...
    createRaceitems: function(raceitems) { //赛程
        ...
        raceitems.forEach(function(val, index) {
            liStr += '<li class="schedule_time clearfix" raceitemId="' + val.raceitemId + '">' +
                ...
                '<span>放飞日期</span>' +
                '<div class="J-datepicker c-datepicker" readonly="readonly">' +
                '<i class=""></i>' +
                '<input type="text" readonly="readonly" autocomplete="off" name="" placeholder="选择日期" class="startedTime" value="' + (val.startedTime || '') + '">' +
                '</div>' +
                '</div>' +
